...
The codebit that does the xor, pairs the two string with the `zip (a,b)` so that each byte from a corresponds to the same byte in b.  
Then the for loop goes over each byte and xors it with the corresponding byte.  
